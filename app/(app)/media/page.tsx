...
const TAG_STYLES: Record<PostTag, { bg: string; text: string; label: string }> = {
  announcement: { bg: "bg-amber-100",  text: "text-amber-700",  label: "Announcement" },
...
const CHALLENGES: Challenge[] = [
  ...
  },
  {
    ...
    leaderboard: [
      ...
      { name: "Sunrise Squad", value: 51, initials: "SS", color: "bg-amber-500" },
  ...
  },
  {
    ...
    podTarget: 10,
    leaderboard: [
      { name: "The Reader's Pod", value: 6, initials: "RP", color: "bg-amber-500" },
...
const AVATAR_COLORS = [
  ...
  "bg-sky-500","bg-violet-500","bg-pink-500","bg-orange-500","bg-teal-500",
]
      ...
      >
        ...
        <div className="p-6">
          {step === "detail" && <>
            ...
            {/* Reward */}
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-6">
              <Trophy size={22} weight="fill" className="text-amber-500 flex-shrink-0" />
              <div>
                <div className="text-xs font-bold text-amber-700 uppercase tracking-wide">Reward</div>
...
function PostCard({ post }: { post: MediaPost }) {
  ...
  return (
    <div className={`bg-white border border-zinc-100 rounded-3xl overflow-hidden shadow-softer hover:shadow-card transition-all duration-300 ${post.pinned ? "ring-1 ring-amber-300/50" : ""}`}>
      {post.image && (
      ...
      <div className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          {post.pinned && <span className="text-[11px] font-semibold text-amber-600">Pinned</span>}
        ...
        {post.cta && (
          <button className="mt-4 flex items-center gap-1.5 text-sm font-semibold text-zinc-900 hover:text-amber-600 transition-colors">
      ...
      {tab === "challenges" && (
        <div className="animate-fade-up" style={{ animationDelay: "80ms" }}>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              { icon: <Lightning size={15} weight="fill" className="text-amber-500" />, value: CHALLENGES.length, label: "Active" },
